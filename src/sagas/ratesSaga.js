...
export function* ratesSaga(useToaster=true) {
  ...
  try{
    ...
    if(useToaster) yield toaster.updateSuccess('Newest rates fetched succesfully');
  }
  catch(err){
    data = mockedRates;
    if(useToaster) yield toaster.updateError(`Unable to fetch rates. Mocked some for you. Error:${err}`);
  }
  finally{
